...
for(const file of await readdir('dist'))if(/^app-.*\.js$/.test(file)&&file!=='app-shift-check.js')await unlink('dist/'+file);
...
await copyFile('dist/app.js','dist/app-shift-check.js');
...
const core=assets.filter(path=>!path.startsWith('./vendor/')&&!path.startsWith('./assets/')&&!path.endsWith('/app.js')&&!path.endsWith('/brand.css'));
await writeFile('dist/sw.js',`const CACHE=${JSON.stringify(version)};const CORE=${JSON.stringify(core)};self.addEventListener('install',event=>event.waitUntil(caches.open(CACHE).then(cache=>cache.addAll(CORE)).then(()=>self.skipWaiting())));self.addEventListener('activate',event=>event.waitUntil(caches.keys().then(keys=>Promise.all(keys.filter(k=>k.startsWith('valida-laser-')&&k!==CACHE).map(k=>caches.delete(k)))).then(()=>self.clients.claim())));self.addEventListener('fetch',event=>{if(event.request.method!=='GET'||new URL(event.request.url).origin!==self.location.origin)return;if(event.request.mode==='navigate'){event.respondWith(fetch(event.request).catch(()=>caches.match('./index.html')));return;}event.respondWith(caches.open(CACHE).then(async cache=>{const cached=await cache.match(event.request,{ignoreSearch:true});if(cached)return cached;const response=await fetch(event.request);if(response.ok)event.waitUntil(cache.put(event.request,response.clone()));return response;}));});`);
