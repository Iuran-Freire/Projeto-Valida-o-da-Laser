...
import {validateSerialPositions} from './serial-profile.mjs';
...
export function inspect(qr,ocr,reliable,visuallyConfirmed=false){
  ...
  const pending=!result.code.serial||!result.print.serial||!reliable&&!visuallyConfirmed||onlyAmbiguousI&&!visuallyConfirmed;
  ...
  return {...result,status:pending?'PENDENTE':profileIssue?'DIVERGENTE':result.status,reason,rawComparison:result.status,ambiguousI};
}
