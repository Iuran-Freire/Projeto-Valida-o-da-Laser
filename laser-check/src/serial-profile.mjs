...
// Tabela do 15W VE TYPE C BLACK. A linha não interfere na conferência do turno.
...
export const CODE_LETTERS_BY_SHIFT=Object.freeze({G:'ADG',H:'BEH',J:'CFJ'});
export const shiftFromCodeLetter=letter=>typeof letter==='string'&&letter.length===1?Object.keys(CODE_LETTERS_BY_SHIFT).find(shift=>CODE_LETTERS_BY_SHIFT[shift].includes(letter))||null:null;
...
export function checkCodeShift(code,serial,shift){
  ...
  return {status:shiftFromCodeLetter(serial[6])===shift?'match':'mismatch',expected:shift,actual:serial[6],actualShift:shiftFromCodeLetter(serial[6])};
}
export function isValidRecordShift(record){
  ...
  return !!serial&&shiftFromCodeLetter(serial[6])===record.shift;
}
export function validateSerialPositions(serial){
  ...
  const positions=Array.from(serial,(value,index)=>{
    ...
    else if(index===6){const shift=shiftFromCodeLetter(value);valid=!!shift;rule='A/D/G=1º, B/E/H=2º, C/F/J=3º turno';if(valid)decoded=SHIFT_LABELS[shift];}
